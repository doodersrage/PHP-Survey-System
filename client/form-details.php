...
$client_form_assign = mysql_query("SELECT client_id FROM client_forms WHERE client_id = '".$_SESSION['client_id']."' AND form_id = '".$form_id."' AND (date_completed IS NOT NULL OR date_completed <> '');");
...
// get form question field data
$modify_query = "SELECT result FROM form_results WHERE field_id = '".$row_questions_field[0]."' AND form_id = '".$form_id."' AND client_id = '".$_SESSION['client_id']."' AND question_id = '".$question_id."';";
$sth = $db->prepare($modify_query);
...
if ($question_count == 0) $form_output .= '<div align="center"><p>We are sorry but no questions have been assigned to the selected form.</p></div>'; else $form_output .= '<div align="center"><input value="Back" type="button" onclick="Javascript: window.history.back();"></div>';
}
